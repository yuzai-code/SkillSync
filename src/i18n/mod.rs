...
pub enum Msg {
    ...
    ListLocalSkillsFound { count: usize },
    ListColName,
    ListColType,
    ListColScope,
    ListColVersion,
    ...
    ReleaseDryRun { old_ver: String, new_ver: String },
    ReleaseVersionUpdated { old_ver: String, new_ver: String },
    ReleaseChangelogUpdated,
    ReleaseCommitCreated,
    ReleaseTagCreated { tag: String },
    ReleasePushed,
    ReleaseTagsPushed,
    ReleaseSuccess { tag: String },

    // ── cli::self_update ────────────────────────────────────────────────
    SelfUpdateChecking,
    SelfUpdateCurrentVersion { version: String },
    SelfUpdateLatestVersion { version: String },
    SelfUpdateAlreadyUpToDate { version: String },
    SelfUpdateDownloading { binary: String },
    SelfUpdateVerifying,
    SelfUpdateChecksumMismatch,
    SelfUpdateUpdated { version: String },
    SelfUpdatePermissionDenied,
    SelfUpdateSudoHint,
    SelfUpdateNetworkError { error: String },
    SelfUpdateRateLimited,
    SelfUpdateNoBinaryForPlatform { platform: String },
...
impl Msg {
    ...
    pub fn en(&self) -> String {
        match self {
            ...
            Msg::ReleaseNotGitRepo => "Not a git repository. Run this command from the project root.".into(),
            Msg::ReleaseWorkingTreeDirty => "Working tree is not clean. Please commit or stash changes first.".into(),
            Msg::ReleaseDryRun { old_ver, new_ver } => format!("Would bump version {} → {}", old_ver, new_ver),
            Msg::ReleaseVersionUpdated { old_ver, new_ver } => format!("Updated version: {} → {}", old_ver, new_ver),
            Msg::ReleaseChangelogUpdated => "Updated CHANGELOG.md".into(),
            Msg::ReleaseCommitCreated => "Created release commit".into(),
            Msg::ReleaseTagCreated { tag } => format!("Created tag: {}", tag),
            Msg::ReleasePushed => "Pushed commit to origin".into(),
            Msg::ReleaseTagsPushed => "Pushed tags to origin".into(),
            Msg::ReleaseSuccess { tag } => format!("Released {} successfully!", tag),

            Msg::SelfUpdateChecking => "Checking for updates...".into(),
            Msg::SelfUpdateCurrentVersion { version } => format!("Current version: {}", version),
            Msg::SelfUpdateLatestVersion { version } => format!("Latest version:  {}", version),
            Msg::SelfUpdateAlreadyUpToDate { version } => format!("Already up to date ({})", version),
            Msg::SelfUpdateDownloading { binary } => format!("Downloading {}...", binary),
            Msg::SelfUpdateVerifying => "Verifying checksum...".into(),
            Msg::SelfUpdateChecksumMismatch => "Checksum verification failed. Download may be corrupted.".into(),
            Msg::SelfUpdateUpdated { version } => format!("Updated to {}", version),
            Msg::SelfUpdatePermissionDenied => "Permission denied. Cannot replace executable.".into(),
            Msg::SelfUpdateSudoHint => "Try: sudo skillsync self update".into(),
            Msg::SelfUpdateNetworkError { error } => format!("Network error: {}", error),
            Msg::SelfUpdateRateLimited => "GitHub API rate limit exceeded. Please try again later.".into(),
            Msg::SelfUpdateNoBinaryForPlatform { platform } => format!("No binary available for platform: {}", platform),
    ...
    pub fn zh(&self) -> String {
        match self {
            ...
            Msg::ReleaseNotGitRepo => "不是 git 仓库。请在项目根目录运行此命令。".into(),
            Msg::ReleaseWorkingTreeDirty => "工作区不干净。请先提交或暂存更改。".into(),
            Msg::ReleaseDryRun { old_ver, new_ver } => format!("将更新版本 {} → {}", old_ver, new_ver),
            Msg::ReleaseVersionUpdated { old_ver, new_ver } => format!("已更新版本：{} → {}", old_ver, new_ver),
            Msg::ReleaseChangelogUpdated => "已更新 CHANGELOG.md".into(),
            Msg::ReleaseCommitCreated => "已创建发布提交".into(),
            Msg::ReleaseTagCreated { tag } => format!("已创建 tag：{}", tag),
            Msg::ReleasePushed => "已推送提交到 origin".into(),
            Msg::ReleaseTagsPushed => "已推送 tags 到 origin".into(),
            Msg::ReleaseSuccess { tag } => format!("成功发布 {}！", tag),

            Msg::SelfUpdateChecking => "正在检查更新...".into(),
            Msg::SelfUpdateCurrentVersion { version } => format!("当前版本：{}", version),
            Msg::SelfUpdateLatestVersion { version } => format!("最新版本：{}", version),
            Msg::SelfUpdateAlreadyUpToDate { version } => format!("已是最新版本（{}）", version),
            Msg::SelfUpdateDownloading { binary } => format!("正在下载 {}...", binary),
            Msg::SelfUpdateVerifying => "正在验证 checksum...".into(),
            Msg::SelfUpdateChecksumMismatch => "Checksum 验证失败。下载文件可能已损坏。".into(),
            Msg::SelfUpdateUpdated { version } => format!("已更新到 {}", version),
            Msg::SelfUpdatePermissionDenied => "权限不足。无法替换可执行文件。".into(),
            Msg::SelfUpdateSudoHint => "请尝试：sudo skillsync self update".into(),
            Msg::SelfUpdateNetworkError { error } => format!("网络错误：{}", error),
            Msg::SelfUpdateRateLimited => "GitHub API 请求频率超限。请稍后重试。".into(),
            Msg::SelfUpdateNoBinaryForPlatform { platform } => format!("当前平台没有可用的二进制文件：{}", platform),
